...
size_t LogManager::log(LogType level, const String& msg) {
    return LogManager::log(level, msg.c_str());
}

size_t LogManager::log(LogType level, const char* msg) {
...
size_t LogManager::log(LogType level, const uint8_t* buf, size_t size) {
...
size_t LogManager::print_tag(LogType level) {
    String msg;
    switch (level) {
        case LogType::DEBUG:
            return 0;
